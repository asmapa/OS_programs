...
void display(int k) {
    if (k == 1 || k == 2 || k == 4) {
        printf("\nProcess    Arrival Time    Burst Time    Waiting Time    Turnaround Time     Response     complete\n");
        for (int i = 0; i < n; i++)
            printf("   P%i\t\t%i\t\t%i\t\t%i\t\t%i\t\t%i\t\t%i\n", process[i], arrival[i], burst[i], waiting[i], turnaround[i],response[i],ct[i]);
    } else {
...
void sjf() {
    ...
    completion[0] = arrival[0] + turnaround[0];
    ...
    for (int i = 1; i < n; i++) {
        ...
        response[val]=waiting[val];
        completion[val] = arrival[val] + turnaround[val];
        ...
        swap(&completion[val],&completion[i]);
        swap(&response[val],&response[i]);
...
void roundrobin() {
    ...
    while (remain() > 0) {
        ...
        if (bt[readyq[front]] <= tq && bt[readyq[front]] > 0 && r > 0) {
              ...
              if (bt[readyq[front]] == burst[readyq[front]]) {
                
                if (waiting[readyq[front]] == -1) {
                    waiting[readyq[front]] = time - arrival[readyq[front]];
                    response[readyq[front]] = waiting[readyq[front]];
                }
            }
